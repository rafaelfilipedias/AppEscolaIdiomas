    ...
    public partial class frmLogin : Form
    {
        ...
        private void btnEntrar_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string codFunc = dt.Rows[0]["codFunc"].ToString();
                int codTipoFunc = int.Parse(dt.Rows[0]["codTipo"].ToString());

                if (codTipoFunc == 1)
                {
                    frmMenuAdm menu = new frmMenuAdm(codFunc);
                    this.Hide();
                    menu.Show();
                }
                else if (codTipoFunc == 2)
                {
                    //fmrMenuAssistente menu = new fmrMenuAssistente(codUsuario);
                    //menu.Show();
                    frmFuncionario func = new frmFuncionario(codFunc);
                    this.Hide();
                    func.Show();
                }
            }
