...
namespace AppEscolaIdiomas
{
    class ClassIdioma
    {
        ...
        public bool Editar(string codIdioma)
        {
            ...
            {
                bd.Conectar();
                bd.ExecutarComandosSql(String.Format("UPDATE Idioma SET nome='{0}', descricao='{1}' WHERE codIdioma = '{2}'",
        ...
        public DataTable RetIdiomaBusca(string cod)
        {
            bd.Conectar();
            DataTable dt = bd.RetDataTable(String.Format("SELECT * FROM Idioma WHERE codIdioma LIKE '%{0}%' OR nome LIKE '%{0}%'", cod));
            bd.Desconectar();
