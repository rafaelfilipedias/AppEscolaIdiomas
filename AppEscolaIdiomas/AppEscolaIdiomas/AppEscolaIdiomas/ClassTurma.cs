...
namespace AppEscolaIdiomas
{
    class ClassTurma
    {
        ...
        {
            bd.Conectar();
            DataTable dt = bd.RetDataTable(String.Format("SELECT * FROM Turma WHERE nome LIKE '{0}'", NomeDigitado));
            bd.Desconectar();
        ...
        public string CodIdiomaDigitado { get; set; }
        public bool ValidarCodIdioma()
        {
            bd.Conectar();
            DataTable dt = bd.RetDataTable(String.Format("SELECT I.codIdioma FROM Idioma AS I INNER JOIN Turma AS T ON (I.codIdioma = T.Idioma_codIdioma) WHERE I.codIdioma LIKE '{0}' ", CodIdiomaDigitado));
            bd.Desconectar();
